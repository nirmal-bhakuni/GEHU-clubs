...
// Simple in-memory notification store (for demo; use a DB in production)
const studentNotifications: Record<string, Array<{ title: string; deadline: string; driveId: string }>> = {};
...
export async function registerRoutes(app: ReturnType<typeof express>): Promise<void> {
    // Student section drives and notifications
    app.get("/api/student/drives", requireStudentAuth, async (req: Request, res: Response) => {
      try {
        const student = (req as any).student;
        if (!student || !student.section) return res.json([]);
        const drives = await Drive.find({ targetSection: student.section }).sort({ createdAt: -1 });
        res.json(drives);
      } catch (error) {
    ...
    app.get("/api/student/notifications", requireStudentAuth, async (req: Request, res: Response) => {
      const student = (req as any).student;
      if (!student) return res.json([]);
      res.json(studentNotifications[student.enrollment] || []);
    });
  ...
  app.post("/api/drive/create", requireFacultyAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!faculty || !faculty.id) {
        console.error("Faculty not found or not authenticated in /api/drive/create", { faculty });
        return res.status(401).json({ error: "Faculty not authenticated or not found" });
      }
      ...
      // Faculty: Generate PDF for section and email students
      app.post("/api/drive/:driveId/section-pdf", requireFacultyAuth, async (req: Request, res: Response) => {
        try {
          const drive = await Drive.findOne({ id: req.params.driveId });
          ...
          doc.on('end', async () => {
            ...
            // Email students (simple text email)
            for (const stu of students) {
              await sendEmail({
                to: stu.email,
                subject: `PDF generated for section ${drive.targetSection}`,
                text: `A PDF with student details for section ${drive.targetSection} has been generated by faculty.`,
              });
            }
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename=section_${drive.targetSection}_students.pdf`);
            res.send(pdfData);
          });
          ...
          res.status(500).json({ error: "Failed to generate PDF or send emails" });
        }
      });
    } catch (error) {
      console.error("Failed to create drive (detailed):", error, {
        body: req.body,
        faculty: (req as any).faculty,
      });
      res.status(500).json({ error: "Failed to create drive", details: error instanceof Error ? error.message : String(error) });
    }
  });
